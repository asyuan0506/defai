...
const securityBadges = ["AES-256-GCM", "HKDF-SHA-256", "隨機 IV", "金鑰不離裝置"];
...
export default function Home() {
  ...
  return (
    <>
      ...
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:rounded-lg focus:bg-amber-500 focus:text-black focus:font-medium"
      >
        Skip to main content
      </a>
      ...
      <div className="min-h-screen bg-[#080E1A] relative overflow-x-hidden">
        ...
        <nav className="relative z-20 max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="size-8 rounded-lg bg-amber-500/15 border border-amber-400/30 flex items-center justify-center">
              <Sparkles aria-hidden="true" className="size-4 text-amber-400" />
            </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-xs text-slate-400 border border-slate-700/60 rounded-full px-3 py-1.5">
              <div aria-hidden="true" className="size-1.5 rounded-full bg-emerald-400 pulse-dot" />
              <span>Devnet</span>
            </div>
            <a
              href="https://github.com"
              aria-label="GitHub 原始碼"
              className="text-slate-500 hover:text-slate-300 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 rounded"
            >
        ...
        <main id="main" className="relative z-10 max-w-6xl mx-auto px-6">
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              <div className="flex flex-wrap gap-3 justify-center lg:justify-start mb-10">
                {stats.map(({ value, label, sub }) => (
                  ...
                  >
                    <p
                      className="font-display font-bold text-white text-base leading-none mb-0.5 tabular-nums"
                    >
                      {value}
                    </p>
              ...
              <a
                href="#how-it-works"
                className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-200 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 rounded"
              >
                了解運作原理
            ...
            <div className="w-full max-w-sm lg:max-w-[360px] flex-shrink-0">
              <div className="glass-card-bright rounded-2xl p-7 border border-slate-700/40 shadow-[0_0_60px_rgba(139,92,246,0.08)]">
                ...
                <div className="flex items-center gap-3 mb-6">
                  <div className="size-10 rounded-xl bg-violet-500/15 border border-violet-400/20 flex items-center justify-center">
                    <Shield aria-hidden="true" className="size-5 text-violet-400" />
                  </div>
                  <div>
                ...
                {publicKey && (
                  <div className="flex items-center gap-2 bg-slate-800/50 border border-slate-700/40 rounded-lg px-3 py-2 mb-4">
                    <div aria-hidden="true" className="size-2 rounded-full bg-emerald-400 shrink-0" />
                ...
                <div className="mt-5 flex flex-col gap-2">
                  {[
                    ...
                    <div key={text} className="flex items-center gap-2 text-xs text-slate-500">
                      <Icon aria-hidden="true" className="size-3.5 text-slate-600 shrink-0" />
                      {text}
                    </div>
                  ))}
                </div>
          ...
          <section className="pb-24">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {features.map(({ icon: Icon, title, desc, color, bg, glow }) => (
                <div
                  key={title}
                  className={`glass-card rounded-2xl p-6 border ${bg} transition-[box-shadow,transform] duration-300 hover:shadow-[0_0_32px_var(--feature-glow)] hover:-translate-y-0.5`}
          ...
          {/* ── How it works ─────────────────────────────────────── */}
          <section id="how-it-works" className="pb-24 scroll-mt-8">
            <div className="text-center mb-12">
              <p className="text-xs font-medium text-slate-500 uppercase tracking-widest mb-3">
                How It Works
              </p>
            ...
            <div className="grid md:grid-cols-3 gap-6 relative">
              ...
              <div
                aria-hidden="true"
                className="hidden md:block absolute top-8 left-[calc(33.33%+1rem)] right-[calc(33.33%+1rem)] h-px bg-gradient-to-r from-amber-400/20 via-emerald-400/20 to-violet-400/20"
              />

              {steps.map(({ num, icon: Icon, title, desc, color, border, iconBg }) => (
                <div key={num} className="relative">
                  <div className={`glass-card rounded-2xl p-7 border ${border} h-full`}>
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`size-10 rounded-xl ${iconBg} border ${border} flex items-center justify-center flex-shrink-0`}>
                        <Icon aria-hidden="true" className={`size-5 ${color}`} />
                      </div>
                      <span aria-hidden="true" className={`font-mono text-xs font-bold ${color} opacity-50`}>{num}</span>
                    </div>
          ...
          <section className="pb-24">
            <div className="glass-card rounded-2xl p-8 border border-emerald-400/10 bg-emerald-500/3">
              <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="flex-shrink-0 float-slow" aria-hidden="true">
                  <div className="size-20 rounded-2xl bg-emerald-400/10 border border-emerald-400/20 flex items-center justify-center">
          ...
          <footer className="pb-10 border-t border-slate-800/60 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="font-display font-bold text-lg text-white opacity-40" aria-label="DeFai">
              De<span className="text-gradient-gold">Fai</span>
            </span>
